...
2. **이 스킬의 `assets/slides-template.md` 를 `src/slides.md` 로 복사** — 검증된
   ...
   들어 있다. 내용만 채우면 완성 형태가 나온다. **영어 발표면 `assets/slides-template-en.md`
   를 복사** (동일 골격, placeholder 만 영어)
...
- mermaid 는 설치 대상이 아니다 — 슬라이드 마지막 장의 `<script>` 가 CDN 에서 로드한다.
  **인터넷이 안 되는 환경**이면 `references/mermaid-pdf.md` 의 로컬 vendoring 절차를 따른다.
...
- 영어 발표는 처음부터 `assets/slides-template-en.md` 를 복사한다 — placeholder 가
  이미 영어 (Agenda / One-slide summary / Current status & roadmap / Thank You)
- 한국어 템플릿으로 시작한 덱을 영어로 바꾸는 경우엔 placeholder·라벨·캡션을 일괄 번역
...
1. frontmatter 바로 뒤 HTML 주석 — `code-baseline: {{sha}} ({{YYYY-MM-DD}})` (템플릿에 포함)
2. 표지 footnote — `코드 기준 {{sha}} ({{YYYY-MM-DD}})`

템플릿의 `{{...}}` placeholder 는 반드시 실값으로 치환한다 — 미치환 placeholder 가
빌드에 남아 있으면 안 된다 (`grep -n "{{" src/slides.md` 로 최종 점검).
...
`htmlLabels: false` 가 필수인 원리(foreignObject 인쇄 경로 버그)와 **CDN 불가 환경의
로컬 vendoring 절차**는 `references/mermaid-pdf.md` (이 SKILL.md 와 같은 디렉토리) 참조.
...
- 다이어그램이 있어야 할 장에 **실제로 그려져 있는가** — CDN 로드 실패 시 조용히 빈
  자리로 빌드된다 (오프라인/사내망이면 `references/mermaid-pdf.md` 의 로컬 vendoring 적용)
...
덱 작업 중 **템플릿/스킬에 없는 스타일·패턴을 수작업으로 추가하게 됐다면** (새 레이아웃
클래스, 반복해서 쓰게 된 슬라이드 구도, 빌드/검증 함정 등), 작업 완료 후 그 델타를
정리해 "이걸 marp-slides 스킬 템플릿에 반영할까요?" 라고 사용자에게 제안한다.
이 스킬의 템플릿 자체가 그런 수작업 반복의 역주입으로 만들어졌다 — 루프를 유지해야
다음 덱이 더 짧은 대화로 완성된다.
